...
describe('PermissionsService', () => {
    ...
    describe('getAllWorkspaceTables', () => {
        it('should return table names filtered by prefix', async () => {
            ...
            expect(mockHttpClient.get).toHaveBeenCalledWith('/api/v2/meta/bases/base_id/tables');
        });
    });
    ...
    describe('getUserPermissions', () => {
        ...
        beforeEach(() => {
            ...
            (nocoDBService.getTableByName as jest.Mock).mockImplementation(async (name) => {
                return { id: `id_${name}`, title: name };
            });
        });

        it('should return cached permissions if available', async () => {
            const cachedPerms = { userId, username: 'cached', roles: [], permissions: new Map() };
        ...
        it('should fetch and calculate permissions if not cached', async () => {
            ...
            expect(result.permissions.get('test_table').has(CrudAction.CREATE)).toBe(true);
            expect(result.permissions.get('test_table').has(CrudAction.DELETE)).toBe(false);
    ...
    describe('canUserPerformAction', () => {
        it('should return true if user has permission', async () => {
            ...
            jest.spyOn(service, 'getUserPermissions').mockResolvedValue({
                userId: 1, username: 'user', roles: [], permissions: perms
            } as any);

            expect(await service.canUserPerformAction(1, 'table1', CrudAction.READ)).toBe(true);
        });
        ...
        it('should return false if user does not have permission', async () => {
            ...
            expect(await service.canUserPerformAction(1, 'table1', CrudAction.CREATE)).toBe(false);
            expect(await service.canUserPerformAction(1, 'other_table', CrudAction.READ)).toBe(false);
        });
    });

    describe('setTablePermissions', () => {
        it('should create permissions if not existing', async () => {
            (nocoDBService.getTableByName as jest.Mock).mockResolvedValue({ id: 'perm_table' });
            mockHttpClient.get.mockResolvedValue({ data: { list: [] } }); // Not found

            await service.setTablePermissions(1, 'table1', { [CrudAction.READ]: true });
    ...
    describe('initializePermissionsForRole', () => {
        it('should iterate all tables and set permissions', async () => {
            jest.spyOn(service, 'getAllWorkspaceTables').mockResolvedValue(['t1', 't2']);
            const setPermSpy = jest.spyOn(service, 'setTablePermissions').mockResolvedValue(undefined);
