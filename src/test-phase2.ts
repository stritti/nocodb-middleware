...
async function testPhase2() {
    ...
    console.log('📋 This test will initialize the app and trigger automatic table creation\n');
    ...
    try {
        ...
        for (const tableName of tablesToCheck) {
            ...
            console.log(`  ${exists ? '✅' : '❌'} Table "${fullName}" ${exists ? 'exists' : 'NOT FOUND'}`);

            if (exists) {
                const table = await nocoDBService.getTableByName(tableName);
                console.log(`     ID: ${table.id}, Columns: ${table.columns?.length || 'unknown'}`);
            }
        }
