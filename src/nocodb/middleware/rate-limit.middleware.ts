...
export class RateLimitMiddleware implements NestMiddleware {
  ...
  private userLimiter = rateLimit({
    windowMs: 15 * 60 * 1000, // 15 minutes
    max: (req: Request) => {
      const user = req.user as RateLimitUser;
    ...
    skip: (req: Request) => {
      // Skip user-based limiting if no user is authenticated
      return !req.user;
    },
    handler: (req: Request, res: Response) => {
      const user = req.user as RateLimitUser;
      this.logger.warn(
        `User rate limit exceeded for user ${user?.id || req.ip} on ${req.originalUrl}`,
      );
      res.status(429).json({
        statusCode: 429,
  ...
  use(req: Request, res: Response, next: NextFunction) {
    ...
    this.userLimiter(req, res, (err: any) => {
      if (err) {
